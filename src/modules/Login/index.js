...
const datas = [
	...
	['谢邀', '人在美国，刚下飞机', '博士学位', '月入过亿', '从小学习舞蹈国画乐器', '家父是一名黑道大哥', '由于某种不幸的原因开始写起了bug', '---------分割线---------', '评论区戾气太重', '圈内人士太多', '利益相关', '匿了', '侵删', '以上'],
    			...
    			<ul>
    				{
    					showDatas.map((item, i) => {
    						return (
    							<li key={ `content${ i }` } style={{ transform: `translateY(${ -(index * 70) }px)`, width: '100%', height: '100%', transition: 'all 0.5s cubic-bezier(0.645, 0.045, 0.355, 1)' }}>{ item }</li>
    						);
    					})
    				}
    			</ul>
