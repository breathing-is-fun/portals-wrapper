...
export interface CurrentModalItem {
  imgurl: string;
  title: string;
  id: string | number;
}
...
export interface PropertyFormState {
  departmentDatas: Array<any>;
}

class PropertyForm extends Component<PropertyFormProps, PropertyFormState> {
  ...
  loadDepartDatas = () => {
    ajax({
      ...
      data: { type: 1 },
      success: ({ data }) => {
        let departmentDatas = handleMenuGroup(data),
          newDepart = [];
        ...
        for (let item of departmentDatas) {
          const { group, groupName, children, id } = item;
